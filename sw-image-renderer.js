...
// The image-renderer service worker:
...
const CACHE_NAME = `molstar-image-renderer-4.10.0`;
...
const APP_STATIC_RESOURCES = [
    ...
    "src/icons/tire.svg",
    "src/icons/wheel.svg",
    "src/examples/image-renderer/index-pwa.html"
];
...
self.addEventListener("install", (event) => {
    event.waitUntil(
        (async () => {
            const cache = await caches.open("molstar-image-renderer-4.10.0");
            await cache.addAll(["/"]);
...
self.addEventListener("activate", (event) => {
    event.waitUntil(
        (async () => {
            ...
            await Promise.all(
                keys.map((key) => {
                    if (key !== "molstar-image-renderer-4.10.0") {
