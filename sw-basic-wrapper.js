...
// The basic-wrapper service worker:
...
const CACHE_NAME = `molstar-basic-wrapper-4.10.0`;
...
self.addEventListener("install", (event) => {
    event.waitUntil(
        (async () => {
            const cache = await caches.open("molstar-basic-wrapper-4.10.0");
            await cache.addAll(["/"]);
...
self.addEventListener("activate", (event) => {
    event.waitUntil(
        (async () => {
            ...
            await Promise.all(
                keys.map((key) => {
                    if (key !== "molstar-basic-wrapper-4.10.0") {
