...
const CACHE_NAME = `molstar-mesoscale-explorer-4.10.0`;
...
self.addEventListener("install", (event) => {
    event.waitUntil(
        (async () => {
            const cache = await caches.open("molstar-mesoscale-explorer-4.10.0");
            await cache.addAll(["/"]);
...
self.addEventListener("activate", (event) => {
    event.waitUntil(
        (async () => {
            ...
            await Promise.all(
                keys.map((key) => {
                    if (key !== "molstar-mesoscale-explorer-4.10.0") {
