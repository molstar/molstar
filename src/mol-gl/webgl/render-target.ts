...
export function createRenderTarget(gl: GLRenderingContext, resources: WebGLResources, _width: number, _height: number, depth = true, type: 'uint8' | 'float32' | 'fp16' = 'uint8', filter: TextureFilter = 'nearest', format: 'rgba' | 'alpha' = 'rgba', depthStencil = false): RenderTarget {
    ...
    const depthRenderbuffer = !depth
        ? null
        : isWebGL2(gl)
            ? resources.renderbuffer(depthStencil ? 'depth32f-stencil8' : 'depth32f', depthStencil ? 'depth-stencil' : 'depth', _width, _height)
