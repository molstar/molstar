...
import { File as H5File } from '../../mol-io/common/h5';
...
/**
 * HDF5 is only a container format - NeXus, EMAN2, Imaris and others all share the
 * same magic bytes and the `.h5`/`.hdf5` extensions. EMDB-SFF mandates a root
 * `primary_descriptor`, so that field is what actually identifies an SFF file.
 */
export function isSffPrimaryDescriptor(value: string | undefined): value is SffPrimaryDescriptor {
    return value === 'three_d_volume' || value === 'mesh_list' || value === 'shape_primitive_list';
}

export async function parseHffData(buffer: ArrayBuffer): Promise<SffData> {
    const file = new (H5File as any)(buffer, '');
    const primaryDescriptor = readString(tryGet(file, 'primary_descriptor'));
    if (!isSffPrimaryDescriptor(primaryDescriptor)) {
        throw new Error(primaryDescriptor === undefined
            ? 'Not an EMDB-SFF file: the HDF5 root has no `primary_descriptor`.'
            : `Not an EMDB-SFF file: unknown \`primary_descriptor\` '${primaryDescriptor}'.`);
    }
    return {
