...
export function convertMvsToMolstar(
    ...
    if (tree.kind === 'root') {
        result.children =
      tree.children?.map((child) => convertMvsNode(child, sourceUrl)) || [];
    } else {
...
function convertMvsNode(node: any, sourceUrl?: string): any {
    ...
    if (node.children) {
        result.children = node.children.map((child: any) =>
            convertMvsNode(child, sourceUrl),
        );
    }
...
export function mvsSanityCheck(tree: MVSTree): void {
    ...
    // Check for common issues
        ...
        if (node.kind === 'component' && node.params?.selector === undefined) {
            issues.push(
                `Component node with undefined selector: ${JSON.stringify(node)}`,
            );
        }
    });
