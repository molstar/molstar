...
import {
  ...
  addSimpleCylinder,
  BasicCylinderProps,
} from "../../../mol-geo/geometry/mesh/builder/cylinder";
...
import { MVSNode } from "molviewspec/tree/mvs/mvs-tree";
import {
  isComponentExpression,
  isPrimitiveComponentExpressions,
  isVector3,
  PrimitivePositionT,
} from "molviewspec/tree/mvs/param-types";
...
type PrimitiveParams<T extends PrimitiveKind = PrimitiveKind> = Extract<
  _PrimitiveParams,
  { kind: T }
>;

export function getPrimitiveStructureRefs(
  primitives: MolstarSubtree<"primitives">,
) {
...
export class MVSPrimitivesData extends SO.Create<PrimitiveBuilderContext>({
  name: "Primitive Data",
  typeClass: "Object",
}) {}
export class MVSPrimitiveShapes extends SO.Create<{
  mesh?: Shape<Mesh>;
  labels?: Shape<Text>;
}>({ name: "Primitive Shapes", typeClass: "Object" }) {}
...
export const MVSDownloadPrimitiveData = MVSTransform({
  ...
  apply({ a, params, cache }, plugin: PluginContext) {
    return Task.create("Download Primitive Data", async (ctx) => {
      ...
      const asset = await plugin.managers.asset
        .resolve(url, "string")
        .runInContext(ctx);
      const node = JSON.parse(
        StringLike.toString(asset.data),
      ) as MolstarSubtree<"primitives">;
...
export const MVSInlinePrimitiveData = MVSTransform({
  ...
  to: MVSPrimitivesData,
  params: {
    node: PD.Value<MolstarSubtree<"primitives">>(undefined as any, {
      isHidden: true,
    }),
  },
})({
...
export const MVSBuildPrimitiveShape = MVSTransform({
  ...
  apply({ a, params, dependencies }) {
    const structureRefs = dependencies
      ? collectMVSReferences([SO.Molecule.Structure], dependencies)
      : {};
      ...
      return new SO.Shape.Provider(
        {
          ...
          params: PD.withDefaults(Mesh.Params, {
            alpha: a.data.options?.opacity ?? 1,
          }),
          getShape: (_, data, __, prev: any) =>
            buildPrimitiveMesh(data, prev?.geometry),
      ...
      return new SO.Shape.Provider(
        {
          ...
          getShape: (_, data, __, prev: any) =>
            buildPrimitiveLabels(data, prev?.geometry),
      ...
      return new SO.Shape.Provider(
        {
          ...
          getShape: (_, data, __, prev: any) =>
            buildPrimitiveLines(data, prev?.geometry),
...
function printEmptySelectionWarning(
  ctx: PrimitiveBuilderContext,
...
function getPrimitives(primitives: MolstarSubtree<"primitives">) {
  return (primitives.children ?? []).filter(
    (c) => c.kind === "primitive",
  ) as MolstarNode<"primitive">[];
}
...
function hasPrimitiveKind(
  context: PrimitiveBuilderContext,
  kind: "mesh" | "line" | "label",
) {
...
function resolvePosition(
  ...
  if (!expr) {
    ...
    throw new Error(
      "Invalid primitive potition expression, see console for details.",
    );
  }

  const pivot = !pivotRef
    ? context.defaultStructure
    : context.structureRefs[pivotRef];
  if (!pivot) {
    throw new Error(
      `Structure with ref '${pivotRef ?? "<default>"}' not found.`,
    );
  }
...
function getInstances(
  options: PrimitivesParams | undefined,
): Mat4[] | undefined {
...
function addMeshWireframe(
  ...
  const { group_colors, group_tooltips, wireframe_color, color, tooltip } =
    params;
  ...
  addMeshFaces(
    ...
    (mvsGroup, builderGroup, a, b, c) => {
      groups.updateColor(
        builderGroup,
        wireframe_color ?? group_colors[mvsGroup] ?? color,
      );
      groups.updateTooltip(builderGroup, group_tooltips[mvsGroup] ?? tooltip);
...
function addLines(
  ...
  for (let i = 0; i < nLines; i++) {
    ...
    groups.updateTooltip(
      builderGroup,
      group_tooltips[mvsGroup] ?? params.tooltip,
    );
...
function resolveLineRefs(
  params: PrimitiveParams<"tube" | "distance_measurement">,
  refs: Set<string>,
) {
  addRef(params.start, refs);
...
function addArrowMesh(
  ...
  const startDefined = resolveBasePosition(
    context,
    params.start,
    ArrowState.start,
  );
  ...
  if (params.end) {
    ...
    console.warn(
      `Primitive arrow does not contain "end" nor "distance". Not showing.`,
    );
    return;
  }
...
function addDistanceMesh(
  ...
  addTubeMesh(context, state, node, { ...params, tooltip } as any, {
    skipResolvePosition: true,
  });
}
...
function addDistanceLabel(
  ...
  if (typeof params.label_size === "number") {
    ...
    size = Math.max(
      dist * params.label_auto_size_scale,
      params.label_auto_size_min,
    );
  }
...
function syncAngleState(
  ...
  AngleState.label =
    typeof params.label_template === "string"
  ...
  if (typeof params.section_radius === "number") {
    ...
    AngleState.radius = Math.min(
      Vec3.magnitude(AngleState.ba),
      Vec3.magnitude(AngleState.bc),
    );
...
function addAngleLabel(
  ...
  if (typeof params.label_size === "number") {
    ...
    size = Math.max(
      AngleState.radius * params.label_auto_size_scale,
      params.label_auto_size_min,
    );
  }
  ...
  labels.add(
    AngleState.label,
    AngleState.labelPos[0],
    AngleState.labelPos[1],
    AngleState.labelPos[2],
    1,
    1,
    group,
  );
...
function getCircle(options: { thetaStart?: number; thetaEnd?: number }) {
  ...
  const thetaLength =
    (options.thetaEnd ?? 2 * Math.PI) - (options.thetaStart ?? 0);
...
function addEllipseMesh(
  ...
  const circle = getCircle({
    thetaStart: params.theta_start,
    thetaEnd: params.theta_end,
  });
  if (!circle) return;
  ...
  Mat4.mul3(
    EllipseState.xform,
    EllipseState.translationXform,
    EllipseState.rotationXform,
    EllipseState.scaleXform,
  );
...
function addEllipsoidMesh(
  ...
  if (typeof params.radius_extent === "number") {
    Vec3.set(
      EllipsoidState.extent,
      params.radius_extent,
      params.radius_extent,
      params.radius_extent,
    );
...
function addBoxMesh(
  ...
  if (params.extent) {
    Box3D.expand(
      BoxState.boundary,
      BoxState.boundary,
      params.extent as unknown as Vec3,
    );
  }
  ...
  Mat4.fromScaling(
    BoxState.scaleXform,
    Box3D.size(BoxState.size, BoxState.boundary),
  );
