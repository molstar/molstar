...
import { Subtree, getChildren } from 'molviewspec/lib/tree/generic/tree-schema';
import { dfs, formatObject } from 'molviewspec/lib/tree/generic/tree-utils';
import {
    MolstarKind,
    MolstarNode,
    MolstarNodeParams,
...
import { DefaultColor } from 'molviewspec/lib/tree/mvs/mvs-tree';
...
): StateTransformer.Params<TransformStructureConformation>[] {
    const result =
    [] as StateTransformer.Params<TransformStructureConformation>[];
    ...
    dfs(tree, (node) => {
        ...
        if (AnnotationFromUriKinds.has(node.kind as any)) {
            ...
            spec = {
                ...
                cifCategory: p.category_name ?? undefined,
            };
    ...
    switch (params.type) {
        ...
        case 'ball_and_stick':
            return {
                type: {
                    name: 'ball-and-stick',
                    params: {
                        sizeFactor: (params.size_factor ?? 1) * 0.5,
                        sizeAspectRatio: 0.5,
                        alpha,
        ...
        case 'carbohydrate':
            return {
                type: {
                    name: 'carbohydrate',
                    params: { alpha, sizeFactor: params.size_factor ?? 1 },
                },
            };
        case 'surface':
            return {
...
export function colorThemeForNode(
    node:
    | MolstarSubtree<
    'color' | 'color_from_uri' | 'color_from_source' | 'representation'
    >
    | undefined,
    context: MolstarLoadingContext,
):
      ...
      hasMolStarUseDefaultColoring(children[0])
            ...
            const layers: MultilayerColorThemeProps['layers'] = children
                .map((c) => {
                    ...
                    return {
                        theme,
                        selection: componentPropsFromSelector(
                            c.kind === 'color' ? c.params.selector : undefined,
                        ),
                    };
                })
                .filter((t) => !!t);
