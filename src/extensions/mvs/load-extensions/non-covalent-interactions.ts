...
import { getCustomProps } from 'molviewspec/lib/tree/generic/tree-schema';
...
export const NonCovalentInteractionsExtension: MolstarLoadingExtension<{}> = {
    ...
    description:
    'Allow showing non-covalent interactions around components with molstar_show_non_covalent_interactions additional property',
    createExtensionContext: () => ({}),
    action: (updateTarget, node, context, extContext) => {
        if (
            node.kind !== 'component' &&
      node.kind !== 'component_from_uri' &&
      node.kind !== 'component_from_source'
        )
            return;
    ...
    const surroundings = UpdateTarget.apply(
        updateTarget,
        StructureSurroundings,
        {
            radius:
          customProps.molstar_non_covalent_interactions_radius_ang ??
          DefaultNonCovalentInteractionRadius,
            includeSelf: true,
            wholeResidues: true,
            nullIfEmpty: false,
        },
