...
import { dfs } from 'molviewspec/tree/generic/tree-utils';
...
export type LoadingActions<TTree extends Tree, TContext> = {
    [kind in Kind<Subtree<TTree>>]?: LoadingAction<
    SubtreeOfKind<TTree, kind>,
    TContext
    >;
};

/** Type for defining custom behavior when loading trees, usually based on node custom properties. */
export interface LoadingExtension<
    TTree extends Tree,
    TContext,
    TExtensionContext,
> {
...
export function loadTreeVirtual<TTree extends Tree, TContext>(
    ...
    const updateRoot: UpdateTarget = UpdateTarget.create(
        plugin,
...
function loadTreeInUpdate<TTree extends Tree, TContext>(
    ...
    dfs<TTree>(tree, (node, parent) => {
        ...
        const action = loadingActions[kind] as
      | LoadingAction<typeof node, TContext>
      | undefined;
        if (action) {
            if (updateParent) {
    ...
    extensionContexts.forEach((e) =>
        e.ext.disposeExtensionContext?.(e.extCtx, tree, context),
    );
}
...
export const UpdateTarget = {
    ...
    apply<A extends StateObject, B extends StateObject, P extends {}>(
        ...
        const apply = target.update
            .to(target.selector)
            .apply(transformer, params, { ...options, ref });
    ...
    dependsOn(target: UpdateTarget, mapping: Map<string, string>): UpdateTarget {
        ...
        const dependsOn = Array.from(target.mvsDependencyRefs)
            .map((d) => mapping.get(d)!)
            .filter((d) => d);
    ...
    deleteChildren(target: UpdateTarget): UpdateTarget {
        const children = target.update.currentTree.children.get(
            target.selector.ref,
        );
