...
import {
  ...
  ParseCcp4,
  ParseCif,
} from "../../mol-plugin-state/transforms/data";
import {
...
} from "./components/custom-label/representation";
import { CustomTooltipsProvider } from "./components/custom-tooltips-prop";
import {
  IsMVSModelProps,
  IsMVSModelProvider,
} from "./components/is-mvs-model-prop";
import {
...
import {
  AnnotationFromSourceKind,
  AnnotationFromUriKind,
...
import { validateTree } from "molviewspec/lib/tree/generic/tree-schema";
import {
  convertMvsToMolstar,
  mvsSanityCheck,
} from "molviewspec/lib/tree/molstar/conversion";
import {
  MolstarNode,
  MolstarNodeParams,
  MolstarSubtree,
  MolstarTree,
  MolstarTreeSchema,
} from "molviewspec/lib/tree/molstar/molstar-tree";
import { MVSTreeSchema } from "molviewspec/lib/tree/mvs/mvs-tree";
...
export async function loadMVS(
  ...
  try {
    ...
    const multiData: MVSData_States =
      data.kind === "multiple" ? data : MVSData.stateToStates(data);
    ...
    for (let i = 0; i < multiData.snapshots.length; i++) {
      ...
      const entry = molstarTreeToEntry(
        ...
        {
          ...snapshot.metadata,
          previousTransitionDurationMs:
            previousSnapshot.metadata.transition_duration_ms,
        },
        options,
      );
      entries.push(entry);
...
function molstarTreeToEntry(
  ...
  const snapshot = loadTreeVirtual(
    plugin,
    tree,
    MolstarLoadingActions,
    context,
    {
  ...
  snapshot.canvas3d = {
    props: plugin.canvas3d
      ? modifyCanvasProps(plugin.canvas3d.props, context.canvas)
      : undefined,
  };
  if (!options?.keepCamera) {
    snapshot.camera = createPluginStateSnapshotCamera(
      plugin,
      context,
      metadata,
    );
  ...
  snapshot.durationInMs =
    metadata.linger_duration_ms + (metadata.previousTransitionDurationMs ?? 0);
  ...
  const entry: PluginStateSnapshotManager.Entry =
    PluginStateSnapshotManager.Entry(snapshot, entryParams);
  return entry;
}
...
export interface MolstarLoadingContext {
  ...
  annotationMap: Map<
    MolstarNode<AnnotationFromUriKind | AnnotationFromSourceKind>,
    string
  >;
...
const MolstarLoadingActions: LoadingActions<
  MolstarTree,
  MolstarLoadingContext
> = {
  root(
    updateParent: UpdateTarget,
    node: MolstarNode<"root">,
  ...
  structure(
    ...
    for (const t of transformProps(node)) {
      transformed = UpdateTarget.apply(
        transformed,
        TransformStructureConformation,
        t,
      ); // applying to the result of previous transform, to get the correct transform order
    }
      ...
      UpdateTarget.apply(struct, CustomStructureProperties, {
        properties: {
          [MVSAnnotationTooltipsProvider.descriptor.name]: {
            tooltips: annotationTooltips,
          },
          [CustomTooltipsProvider.descriptor.name]: {
            tooltips: inlineTooltips,
          },
        },
        autoAttach: [
          MVSAnnotationTooltipsProvider.descriptor.name,
  ...
  component_from_uri(
    ...
    return UpdateTarget.apply(
      updateParent,
      MVSAnnotationStructureComponent,
      props,
    );
  },
  ...
  volume(
    updateParent: UpdateTarget,
    node: MolstarNode<"volume">,
  ): UpdateTarget | undefined {
    if (
      updateParent.transformer?.definition.to.includes(
        PluginStateObject.Format.Ccp4,
      )
    ) {
      return UpdateTarget.apply(updateParent, VolumeFromCcp4, {});
    } else if (
      updateParent.transformer?.definition.to.includes(
        PluginStateObject.Format.Cif,
      )
    ) {
      return UpdateTarget.apply(updateParent, VolumeFromDensityServerCif, {
  ...
  focus(
    updateParent: UpdateTarget,
    node: MolstarNode<"focus">,
    context: MolstarLoadingContext,
  ): UpdateTarget {
    context.camera.focuses.push({
      target: updateParent.selector,
      params: node.params,
    });
    return updateParent;
  },
  ...
  primitives(
    ...
    const data = UpdateTarget.apply(updateParent, MVSInlinePrimitiveData, {
      node: tree,
    });
