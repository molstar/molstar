...
import {
  createMVSBuilder,
  Structure as MVSStructure,
  Representation,
  Root,
} from "molviewspec/tree/mvs/mvs-builder";
import { MVSNodeParams } from "molviewspec/tree/mvs/mvs-tree";
import {
  ColorT,
...
const Superpositions = {
  "1opl": [
    -0.6321036327, 0.3450463255, 0.6938213248, 0, -0.6288677634, -0.7515716885,
    -0.1991615756, 0, 0.4527364948, -0.5622126202, 0.6920597055, 0,
    36.3924122492, 118.2516908402, -26.4992054179, 1,
  ] as unknown as Mat4,
  "3ik3": [
    -0.7767826245, -0.6295936551, 0.0148520572, 0, 0.6059737752, -0.7408035481,
    0.2898376906, 0, -0.1714775143, 0.2341408391, 0.9569605684, 0,
    21.0648276775, 53.0266628762, -0.3385906075, 1,
  ] as unknown as Mat4,
  "2gqg": [
    0.0648740828, -0.7163272638, 0.6947421137, 0, 0.0160329972, -0.6953706204,
    -0.7184724374, 0, 0.9977646498, 0.0577490387, -0.0336266582, 0,
    -31.0690973964, 146.0940883054, 39.7107422531, 1,
  ] as unknown as Mat4,
  "2g2i": [
    -0.5680242227, 0.6527660987, 0.5012433569, 0, -0.10067389, 0.5493518768,
    -0.8295042395, 0, -0.8168312251, -0.5216406194, -0.2463286704, 0,
    -8.1905690894, 75.7603329146, -6.1327389269, 1,
  ] as unknown as Mat4,
  "3oxz": [
    0.7989033646, 0.5984398921, -0.0601922711, 0, -0.1303123126, 0.269921501,
    0.9540236289, 0, 0.5871729857, -0.754328893, 0.2936252816, 0, -8.0697093741,
    58.1709160658, 19.0363028443, 1,
  ] as unknown as Mat4,
};
...
`,
    state: () => {
      ...
      const _1opl = structure(builder, "1opl");
      const [_1opl_poly, _1opl_poly_repr] = polymer(_1opl, {
        color: Colors["1opl"],
      });
    ...
    state: () => {
      ...
      domains(
        ...
        [
          ...
          [
            Domains.Activation_loop,
            DomainColors["Activation_loop"],
            "Activation Loop (active)",
            { label_size: 3 },
          ],
        ],
        { label_size: 3 },
      ...
      drawInteractions(_2g2i, [
        [
          "Salt Bridge",
          { auth_asym_id: "A", auth_seq_id: 271, auth_atom_id: "NZ" },
          { auth_asym_id: "A", auth_seq_id: 286, auth_atom_id: "OE1" },
          { skipResidue: true },
        ],
      ]);
...
`,
    state: (): Root => {
      return Steps.find(
        (s: any) => s.key === "unstoppable-signaling",
      )?.state()!;
    },
    camera: {
...
`,
    state: () => {
      ...
      drawInteractions(_1iep, [
        [
          "H-bond",
          { auth_asym_id: "A", auth_seq_id: 286, auth_atom_id: "OE2" },
          { label_asym_id: "G", label_atom_id: "N21" },
          { skipResidue: true },
        ],
        [
          "H-bond",
          { auth_asym_id: "A", auth_seq_id: 315, auth_atom_id: "OG1" },
          { label_asym_id: "G", label_atom_id: "N13" },
          { skipResidue: true },
        ],
        [
          "H-bond",
          { auth_asym_id: "A", auth_seq_id: 318, auth_atom_id: "N" },
          { label_asym_id: "G", label_atom_id: "N3" },
          { skipResidue: true },
        ],
        [
          "H-bond",
          { auth_asym_id: "A", auth_seq_id: 318, auth_atom_id: "O" },
          { label_asym_id: "G", label_atom_id: "N3" },
          { skipResidue: true },
        ],
        [
          "H-bond",
          { auth_asym_id: "A", auth_seq_id: 360, auth_atom_id: "O" },
          { label_asym_id: "G", label_atom_id: "N51" },
          { skipResidue: true },
        ],
        [
          "H-bond",
          { auth_asym_id: "A", auth_seq_id: 361, auth_atom_id: "O" },
          { label_asym_id: "G", label_atom_id: "N51" },
          { skipResidue: true },
        ],
        [
          "H-bond",
          { auth_asym_id: "A", auth_seq_id: 381, auth_atom_id: "N" },
          { label_asym_id: "G", label_atom_id: "O29" },
          { skipResidue: true },
        ],
      ]);
    ...
    state: () => {
      ...
      drawInteractions(_3oxz, [
        [
          "H-bond",
          { auth_asym_id: "A", auth_seq_id: 360, auth_atom_id: "O" },
          { label_asym_id: "B", label_atom_id: "N4" },
          { skipResidue: true },
        ],
        [
          "H-bond",
        ...
        [
          "H-bond",
          { auth_asym_id: "A", auth_seq_id: 286, auth_atom_id: "OE2" },
          { label_asym_id: "B", label_atom_id: "N2" },
          { skipResidue: true },
        ],
        [
          "H-bond",
        ...
        [
          "H-bond",
          { auth_asym_id: "A", auth_seq_id: 318, auth_atom_id: "N" },
          { label_asym_id: "B", label_atom_id: "N1" },
          { skipResidue: true },
        ],
      ]);
...
type Interaction = [
  label: string,
  polymer: PrimitivePositionT,
  ligand: PrimitivePositionT,
...
function drawInteractions(
  ...
  for (const [tooltip, a, b, options] of interactions) {
    ...
    const expressions = isPrimitiveComponentExpressions(a)
      ? a.expressions!
      : [a as ComponentExpressionT];
...
function domains(
  ...
  domains: [
    selector: ComponentExpressionT,
    color: ColorT,
    label?: string,
    options?: { label_size?: number },
  ][],
  options?: { label_size?: number },
) {
  ...
  for (const [selector, color, label, opts] of domains) {
    ...
    if (label)
      primitives!.label({
        ...
        text: label,
        label_color: color,
        label_size: opts?.label_size ?? options?.label_size ?? 1.5,
      });
  }
}
...
function ligand(
        ...
        custom: {
          ...
          molstar_color_theme_params: {
            carbonColor: options?.carbon_color
              ? {
                  name: "uniform",
                  params: { value: decodeColor(options?.carbon_color) },
                }
  ...
  const label_color: ColorT =
    options?.label_color ??
    options.uniform_color ??
    options.carbon_color ??
    "#5B53A4";
  if (options.label) {
    structure.primitives().label({
      position: Array.isArray(options.selector)
...
function bindingSite(
  structure: MVSStructure,
  residues: [selector: ComponentExpressionT, label: string][],
  options: {
    color?: ColorT;
    label_size?: number;
  },
  ...
  structure
    .component({ selector: residues.map((r) => r[0]) })
    .representation({ type: "ball_and_stick" })
    .color(coloring);
