...
import { createMVSBuilder } from "molviewspec/tree/mvs/mvs-builder";
...
} from "../../mol-model/structure/model/properties/coarse";
...
function getCoarseElementPosition(
  e: CoarseElementReference,
  model: Model,
  position: Vec3,
) {
...
async function parseInfo(
  ...
  const ihm_cross_link_restraint =
    dataBlocks[0].categories["ihm_cross_link_restraint"];
  ...
  for (let modelIndex = 0; modelIndex < trajectory.frameCount; modelIndex++) {
    ...
    for (let i = 0; i < ihm_cross_link_restraint.rowCount; i++) {
      ...
      if (
        !resolvePosition(model, e1key, a) ||
        !resolvePosition(model, e2key, b)
      ) {
        continue;
      }

      const restraintType: "harmonic" | "upper bound" | "lower bound" =
        restraint_type.str(i)?.toLowerCase() as any;
