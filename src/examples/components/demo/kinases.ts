...
// type Domain = [label: string, color: ColorT, domain: PrimitivePositionT];

const Domains = {
    '1opl': {
        SH2: { auth_asym_id: 'A', beg_auth_seq_id: 146, end_auth_seq_id: 247 },
        SH3: { auth_asym_id: 'A', beg_auth_seq_id: 83, end_auth_seq_id: 145 },
        NTerm: { auth_asym_id: 'A', beg_auth_seq_id: 259, end_auth_seq_id: 344 },
        CTerm: { auth_asym_id: 'A', beg_auth_seq_id: 345, end_auth_seq_id: 519 },
    },
    '1iep': {
...
`,
        state: () => {
            ...
            _1opl
                ...
                .color({ color: '#55ACAE', selector: Domains['1opl'].SH3 })
                .color({ color: '#A3DAA3', selector: Domains['1opl'].SH2 });

            _1opl.component({ selector: Domains['1opl'].SH3 }).label({ text: 'SH3' });
            _1opl.component({ selector: Domains['1opl'].SH2 }).label({ text: 'SH2' });
...
`,
        ...
        camera: {
            position: [-34.47, -61.4, 21.49],
            target: [-0.52, 44.34, 14.97],
            up: [-0.15, -0.01, -0.99],
        } satisfies MVSNodeParams<'camera'>,
    }, {
...
`,
        state: () => {
            ...
            _1iep_polymer
                .representation()
                .color({ color: Colors.background })
                .color({ color: 'lightgray', selector: Domains['1iep'].N_lobe })
                .color({ color: 'darkgray', selector: Domains['1iep'].C_lobe })
                .color({ color: 'pink', selector: Domains['1iep'].P_loop })
                .color({ color: 'red', selector: Domains['1iep'].Action_loop })
                .color({ color: 'orange', selector: Domains['1iep'].DFG_motif });

            const _1iep_labels = _1iep.primitives();

            _1iep_labels.label({ position: Domains['1iep'].P_loop, text: 'P-loop', label_size: 3, label_color: 'pink' });
            _1iep_labels.label({ position: Domains['1iep'].N_lobe, text: 'N-lobe', label_size: 3, label_color: 'lightgray' });
            _1iep_labels.label({ position: Domains['1iep'].C_lobe, text: 'C-lobe', label_size: 3, label_color: 'darkgray' });
            _1iep_labels.label({ position: Domains['1iep'].Action_loop, text: 'Activation Loop', label_size: 3, label_color: 'red' });
            _1iep_labels.label({ position: Domains['1iep'].DFG_motif, text: 'DFG Motif', label_size: 3, label_color: 'orange' });
...
`,
        state: () => {
            ...
            const _1iep_ligand = _1iep.component({ selector: ligand });
            _1iep_ligand.representation({ type: 'surface' }).color({ custom: { molstar_color_theme_name: 'element-symbol' } }).opacity({ opacity: 0.33 });
            _1iep_ligand.representation({ type: 'ball_and_stick' }).color({ custom: { molstar_color_theme_name: 'element-symbol' } });
            ...
            const _3ik3_mutation = _3ik3.component({ selector: mutation });
            _3ik3_mutation.representation({ type: 'spacefill' }).color({ color: 'red' }).opacity({ opacity: 0.33 });
            _3ik3_mutation.representation({ type: 'ball_and_stick' }).color({ custom: { molstar_color_theme_name: 'element-symbol' } });
...
function carbonColor(hexColor: string) {
    return { molstar_color_theme_params: { carbonColor: { name: 'uniform', params: { value: Color.fromHexStyle(hexColor) } } } };
}
...
export function buildStory(): MVSData_States {
    const snapshots = Steps.map((s, i) => {
        ...
        const description = i > 0 && i < Steps.length - 1 ? `${s.description}\n\n[Go to start](#intro)` : s.description;
