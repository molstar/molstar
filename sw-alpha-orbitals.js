...
// The alpha-orbitals service worker:
...
const CACHE_NAME = `molstar-alpha-orbitals-4.10.0`;
...
self.addEventListener("install", (event) => {
    event.waitUntil(
        (async () => {
            const cache = await caches.open("molstar-alpha-orbitals-4.10.0");
            await cache.addAll(["/"]);
...
self.addEventListener("activate", (event) => {
    event.waitUntil(
        (async () => {
            ...
            await Promise.all(
                keys.map((key) => {
                    if (key !== "molstar-alpha-orbitals-4.10.0") {
