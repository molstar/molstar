...
function generateServiceWorkerFile(name) {
    ...
    const filePath = path.join(__dirname, '..', 'lib', `${name}`, `sw.js`);
    // Set faviconpath.
    // Initialise content.
...
function generateManifestFile(name) {
    const filePath = path.join(__dirname, '..', 'lib', `${name}`, 'manifest.webmanifest');
    const content=`{
...
function processHtmlFile(name) {
    ...
    let outDirPath = path.join(__dirname, '..', 'lib', 'apps', `${name}`);

    ensureDirectoryExistence(outDirPath);
    
    // Define the source and destination file paths
    let inFilePath = path.join(__dirname, '..', 'src', 'apps', `${name}`, 'index.html');
    let outFilePath = path.join(outDirPath, 'index-pwa.html');
