...
import { canonicalJsonString } from '../../util/json';
...
export function treeToString(tree: Tree) {
    let level = 0;
    const lines: string[] = [];
    dfs(tree, node => lines.push('  '.repeat(level++) + nodeToString(node)), _node => level--);
    return lines.join('\n');
}
...
function windowUrl(): string | undefined {
    return (typeof globalThis !== 'undefined' && 'location' in globalThis) ? (globalThis as any).location.href : undefined;
}
